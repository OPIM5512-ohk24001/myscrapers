...
def _list_run_ids(bucket, structured_prefix):
    it = storage_client.list_blobs(bucket, prefix=structured_prefix + "/", delimiter="/")
    for _ in it:
        pass
    run_ids = []
...
def _jsonl_records_for_run(bucket, structured_prefix, run_id):
    b = storage_client.bucket(bucket)
    prefix = structured_prefix + "/run_id=" + run_id + "/jsonl/"
...
def _write_csv(records, dest_key, columns=None):
    if columns is None:
        columns = CSV_COLUMNS
    n = 0
...
def materialize_http(request):
    try:
        ...
        final_key = STRUCTURED_PREFIX + "/datasets/listings_v2.csv"
        ...
        return jsonify({
            ...
            "rows_written": rows,
            "output_csv": "gs://" + BUCKET_NAME + "/" + final_key
        }), 200
